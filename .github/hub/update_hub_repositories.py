# adapted from https://github.com/huggingface/datasets/blob/master/.github/hub/update_hub_repositories.py
...
def canonical_dataset_info_url(dataset_name: str, organization: str) -> str:
    return HUB_CANONICAL_INFO_URL.format(dataset_name=dataset_name, organization=organization)


def create_remote_repo(dataset_name: str, token: str, organization: str):
...
def check_authorizations(user_info: dict, organization: str):
    if organization not in [org["name"] for org in user_info["orgs"] if org["type"] == "org"]:
    ...
    def __call__(self, dataset_name: str) -> bool:
        ...
        if not canonical_dataset_path(dataset_name).is_dir():
            repo = Repo.clone_from(
                canonical_dataset_git_url(dataset_name, self.token, self.organization),
...
if __name__ == "__main__":
    ...
    check_authorizations(user_info, organization)
